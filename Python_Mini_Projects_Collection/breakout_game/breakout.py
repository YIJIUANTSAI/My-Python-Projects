...
def main():
    ...
    while True:
        ...
        # check the wall edge
        if graphics.ball.x <= 0 or graphics.ball.x+graphics.ball.width >= graphics.window.width:  # 左半邊 or 右半邊
            graphics.set_vx(-vx)
        if graphics.ball.y <= 0:  # 上半部
            graphics.set_vy(-vy)
        # 下半部 & avoid the ball stuck bouncing in the paddle
